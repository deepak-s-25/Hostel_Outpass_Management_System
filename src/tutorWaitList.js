...
const AddStudent = () => {
    ...
    const handleClick = (rid)=>{
        console.log(rid);
        axios.post("http://localhost:8081/saveTutorApprove", {
            rid: rid
        }).then(response=> {
    ...
    useEffect(() => {
        ...
        try {
            axios.post("http://localhost:8081/getPasswaitforTutor", {
                    ...
                    <table className="table table-text-small mb-0">
                        ...
                        <tbody>
                            {FilteredData().map((data, index) => {
                                const { rid, file, sname, rollno, wname, reason, destination, from_date, to_date,p_name,address,mobile } = data;
                                return (
                                    <tr key={index}>
                                        <td>{index+1}</td>
                                        ...
                                        <td><button onClick={() => handleClick(rid)} style={{ backgroundColor: 'green', borderRadius: '10%', padding: '8px', color: 'white' }}>Approve</button></td>
                                    </tr>
                                );
                            })}
                        </tbody>
